...
contract LasVulvas is ERC721Pausable, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    uint256 private constant _maxTokens = 1000; // 1000 maximum tokens
    ...
    constructor() ERC721("Las Vulvas", "LVS") {
        _pause();
    }
    ...
    function mintItems(uint256 amount) public payable {
        require(amount <= _maxMint); //, "No more minting possible");
        require(_saleActive); //, "Sale is not active");

        uint256 totalMinted = _tokenIds.current();
        require(totalMinted + amount <= _maxTokens); //, "Too many token to mint");
    ...
    function reserve(uint256 quantity) public onlyOwner {
        for (uint256 i = _tokenIds.current(); i < quantity; i++) {
            if (i < _maxTokens) {
                _tokenIds.increment();
